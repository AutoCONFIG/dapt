...
def rotation_matrix_to_angle_axis(rotation_matrix):
    ...
    if rotation_matrix.shape[1:] == (3,3):
        ...
        ).reshape(1, 3, 1).expand(rot_mat.shape[0], -1, -1)
...
def get_pred_poses(pred_rotmats):
    pred_poses = []
    for pred_rotmat in pred_rotmats:
        pred_poses.append(
            rotation_matrix_to_angle_axis(torch.from_numpy(pred_rotmat)).numpy().reshape((72, ))
        )

    pred_poses = np.stack(pred_poses)
    return pred_poses

def get_gt_poses(pose_folder):
    """
        pose_folder: config.poser_folder: '/home/wangxin/data/lidarcap/pose/'
    """
    gt_poses = []
    for index in [7, 24, 29, 41]:
        pose = '{}{}'.format(pose_folder, index)
        for pose_filename in filter(lambda x: x.endswith('.json'), get_sorted_filenames_by_index(pose)):
...
def get_gt_poses_2(pose_folder = '/data/wx/data/lidarcap/pose/', index = 7):
    """
        pose_folder: config.poser_folder: '/home/wangxin/data/lidarcap/pose/'
    """
    gt_poses = []
...
def get_sorted_filenames_by_index(dirname, isabs=True):
    filenames = sorted(
        list(filter(is_index_filename, os.listdir(dirname))),
