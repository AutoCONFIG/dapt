...
from pointcept.models.point_transformer_v3.point_transformer_v3m1_base import Block
...
class LPFormer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        bs = point.offset.shape[0]
        local_points = point.coord.reshape(bs, -1, 3)
        points_feat = torch.cat([local_points, local_points], dim = -1)
        voxel_feat = self.get_voxel_(local_points)
        feat0 = self.mlp_(torch.cat([points_feat, voxel_feat], dim = -1)) #.float()
        query = self.kpt_embedding(torch.arange(self.num_kp).to(points_feat.device)).unsqueeze(0).repeat(bs, 1, 1)#self.kpt_querys.unsqueeze(0).repeat(bs, 1, 1)
    ...
    def get_voxel_(self, local_points):
        #[B,N,3]
        B,N = local_points.shape[:2]
        voxel_input = torch.zeros((B,) + (1,) + self.voxel_size).to(local_points.device)
