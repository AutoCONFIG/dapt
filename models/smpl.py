...
ProjectPoints, ColoredRenderer, LambertianPointLight = None, None, None  # 考虑到opendr不易安装,暂时注释掉


class SMPL(nn.Module):

    def __init__(self, 
                 smpl_model='smpl_models/smpl/SMPL_NEUTRAL.pkl',
                 j_regressor_extra='smpl_models/J_regressor_extra.npy'):
        super(SMPL, self).__init__()
        with open(smpl_model, 'rb') as f:
        ...
        J_regressor_extra = torch.from_numpy(np.load(j_regressor_extra)).float()
    ...
    def forward(self, pose, beta):  # return vertices location
        ...
        # input it rotmat: (bs,72)
            ...
            R = geometry.batch_rodrigues(pose_cube).view(batch_size, 24, 3, 3)
